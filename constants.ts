...
【精确时长控制公式】（严格遵守，这是最高优先级规则）
- 冥想TTS朗读速度约 120 字/分钟（比正常语速慢很多，因为冥想需要缓慢深沉的节奏）
- 每段 pauseSeconds 也计入总时长
- 公式：总字数 = (目标分钟数 - 总pauseSeconds/60) x 120
- 具体字数硬限制（必须严格遵守，不得超出）：
  * 3 分钟：总字数 250-320 字，pauseSeconds 合计 15-25 秒
  * 5 分钟：总字数 400-500 字，pauseSeconds 合计 30-50 秒
  * 10 分钟：总字数 800-1000 字，pauseSeconds 合计 60-100 秒
  * 15 分钟：总字数 1200-1500 字，pauseSeconds 合计 90-150 秒
  * 20 分钟：总字数 1600-2000 字，pauseSeconds 合计 120-200 秒
- 【重要】：宁可字数略少也不要超出上限！内容质量远比篇幅重要
- 【重要】：请在构思前先计算目标字数，写完后验证总字数是否在范围内

【段落配比指南（按目标时长自适应调整）】
- 3 分钟：2-3 段（intro+breathing 合并 + visualization/核心 + outro），每段精炼，不用面面俱到
- 5 分钟：3-4 段（intro + breathing + visualization + outro）
...
【关键声音参数锁定】
1. **语速控制**: 请将语速降低至正常语速的 70%-75% (约每分钟 110-130 字)。字与字之间留出微小的缝隙，句子结尾处要有明显的自然下沉和呼吸感。
...
export const QWEN_VOICES = [
  ...
  // —— Qwen3-TTS Instruct-Flash 官方音色 ——
  // 使用与 CosyVoice 相同的标准 inference 协议端点渲染
  { id: 'Seren', name: '晚安好梦 (小婉)', gender: 'female', model: 'qwen3-tts-instruct-flash' },
  { id: 'Serena', name: '温柔小姐姐 (苏瑶)', gender: 'female', model: 'qwen3-tts-instruct-flash' },
  { id: 'Ethan', name: '阳光温暖 (晨煦)', gender: 'male', model: 'qwen3-tts-instruct-flash' },
  { id: 'Maia', name: '知性温柔 (四月)', gender: 'female', model: 'qwen3-tts-instruct-flash' },
];
